...
float4 GetSSR(sampler2D grabPass, float4 texelSize, const float3 wPos, const float3 viewDir, 
	float3 rayDir, const half3 faceNormal, float smoothness, float2 screenUVs, float4 screenPos,
	float fadeFactor)
	{
	...
	if (SSR_IsInMirror() || FdotR < 0){
		...
		noiseUvs.xy = (noiseUvs.xy * texelSize.zw) / (_NoiseTexSSR_TexelSize.zw * noiseUvs.w);	
		...
		uvs.xy = uvs.xy / uvs.w;
			...
			float xfade = 1;
		#else
			float xfade = smoothstep(0, fadeFactor, uvs.x) * smoothstep(1, 1-fadeFactor, uvs.x); //Fade x uvs out towards the edges
		#endif
		float yfade = smoothstep(0, fadeFactor, uvs.y)*smoothstep(1, 1-fadeFactor, uvs.y); //Same for y
		...
		float4 reflection = float4(GetBlurredGP(grabPass, texelSize.zw, uvs.xy, blurFac*1.5),1);	
